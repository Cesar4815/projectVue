<template>
  <div>
    <v-toolbar flat height=90>
      ...
      {{ this.volume * 100 + '%' }}
      <v-spacer></v-spacer>
      <v-btn outline  small color="green" @click="skipTrack('prev')">
        <v-icon>skip_previous</v-icon>
      </v-btn>
      <v-btn outline  small color="green" @click="stopTrack">
        <v-icon>stop</v-icon>
      </v-btn>
      <v-btn outline  small color="green" @click="playTrack()">
        <v-icon >play_arrow</v-icon>
      </v-btn>
      <v-btn outline  small color="green" @click="pauseTrack">
        <v-icon>pause</v-icon>
      </v-btn>
      <v-btn outline  small color="green" @click="skipTrack('next')">
        <v-icon>skip_next</v-icon>
      </v-btn>
      <v-spacer></v-spacer>
      <v-btn flat icon @click="toggleLoop">
        <v-icon color="green" v-if="this.loop">repeat_one</v-icon>
        <v-icon color="blue-grey" v-else>repeat_one</v-icon>
      </v-btn>
      <v-btn flat icon @click="toggleShuffle">
        <v-icon color="green" v-if="this.shuffle">shuffle</v-icon>
